...
Javascript files, it displays an edition.
...
def putin_prose_line(line, chapter, vsnum, pada, indent, uvacaflag, proseblocknum):
    """
    Puts all the required html stuff into the main text
    """
    uvaca_or_not = "<uvaca>" * uvacaflag
...
def main(filename):
    ...
    """
       ...
       <body onload="loaded();">
       <span style="font-size:100%; cursor:pointer; color:#cc8800; float:left;" onclick="openNavLeft()"><b>&#10095;</b></span>
        <div id="mySidenavLeft" class="sidenavleft">
          <a href="javascript:void(0)" class="closebtnleft" onclick="closeNavLeft()">&times;</a>
  <span class="instr">
        <b>&#10086; HOW TO USE THIS PAGE &#10086;</b> <br/><br/>
        &#9758; The three windows are draggable and resizable<br/>
         &#9758; Press keys 's', 't', and 'n' to toggle the Sanskrit text,
         the translation and the notes, respectively (the last one popping up will take the foreground, potentially overlapping the others);
         press 'o' to open/close all apparatus entries; press 'd' to toggle Roman/Devanāgarī<br/>
         &#9758; You can also use these buttons for the same:<br/>
          <button onclick="togglearea('translation')" id="showtextandtranslation">Toggle Translation</button><br/>
          <button onclick="togglearea('notes')"       id="txttrnotes">Toggle Notes</button><br/>
          <button onclick="toggleallapp()"              id="toggleallapp">Open all apparatus entries</button><br/>
          <button onclick="turnItDevnag()"            id="switchbutton">Switch to Devanāgarī</button><br/>
          &#9758; Single click on Sanskrit line to scroll to relevant note, if any, and if notes are being displayed<br/>
          &#9758; Double click on Sanskrit line to display apparatus and highlight translation of verse (if translation is visible)<br/>
          &#9758; Click inside apparatus box to close it<br/>
          &#9758; Double click on translation to toggle highlighting on relevant Sanskrit verse<br/>
          &#9758; Hovering over a siglum in the apparatus that has the ∑ sign will show the underlying MSS sigla<br/>
     </span>
     </div>

          <span style="font-size:100%; cursor:pointer; color:#cc8800; float:right;" onclick="openNavRight()"><b>&#10094;</b></span>
    ...
    print('<a href="'+re.sub("\..*", "", filename)+'_metadata.html" target="_blank">Metadata &#9758;</a>')
    print('<a href="index.html">Manuscript images &#9758;</a>')
    print('<a href="'+re.sub("\..*", "", filename)+'.pdf" target="_blank">Printed edition &#9758;</a>')
    print(
    """
            <a href="index.html">Parallel texts &#9758;</a>
            <a href="index.html">Bibliography &#9758;</a>
            <a href="index.html">Acknowledgements &#9758;</a>
    ...
    with open(filename, "r", encoding="utf-8") as openfile:
        for line in openfile:
            ...
            if onflag is True:
                ...
                # checking if <TEXT>... is the first in the file
                ...
                if TEXT_OPEN_TAG in line or textflag is True:
                    ...
                    elif "|*" in line and proseflag is False:
                        # main process to put in stuff
                        uvacaflag = 1
                        pada = "uvaca"
                        indent = 0
                    ...
                    # with anuṣṭubh, a danda increases verse number if it is the first single danda
                        ...
                        hemistich = 1
                    # check if this is a non-anuṣṭubh 1st, 3rd or 5th line
                    elif "|" not in line and anustubh is False and proseflag is False:
                        # no indent
                        # main process to put in stuff
                        uvacaflag = 0
                        if hemistich == 0: 
                    ...
                    outputline = re.sub(
                        ...
                        + "."
                        + str(vsnum + uvacaflag)
                        + pada * abs(proseflag-1)
                        + ("prose" + str(proseblocknum)) * proseflag
                        + '">',
                        outputline
                    )
                    ...
                    if proseflag is True:
                        outputline = re.sub(
                            TEXT_OPEN_TAG, "<TEXT>", outputline
                        )
                        outputline = putin_prose_line(
                            outputline, chapter, vsnum, pada, indent, uvacaflag, proseblocknum
                        )
                        outputline = re.sub(
                            TEXT_CLOSING_TAG, "</proseblock>", outputline
                        )
                    #print(outputline, end="")
                    collected_text = collected_text + outputline
                ...
                if APP_OPEN_TAG in line or appflag is True:
                    ...
                    # ignore 'hide pāda' instruction
                    outputline = re.sub("<LEM!>", '<LEM>', outputline)
                    ...
                    if 'mssALL' in dn_output_line:
                        dn_output_line = re.sub('mssALL', '<div class="tooltip">\\\\mssALL<span class="tooltiptext">', dn_output_line)
                        dn_output_line = re.sub(';', "</span></div>;", dn_output_line)
                ...
                if '<PVAR>' in line or pvarflag is True:
                    ...
                    outputline = xml_substitutions.xml_substitutions(outputline)
                    dn_output_line = re.sub('\|', ",", dn_output_line)
                    dn_output_line = toDevanagariExceptTagsAndCommands.main(dn_output_line)
                    dn_output_line = xml_substitutions.xml_substitutions(dn_output_line)
                    print(
                        "<RMAPP>"
                        + outputline
                ...
                if "<TR>" in line or trflag is True:
                    ...
                    strng = "sktvrs" + str(chapter) + "." + str(vsnum + uvacaflag) + ('prose' + str(proseblocknum))*proseflag
                    outputline = re.sub(
                        "<?!?-?-? ?<TR>",
                        '\n<trnsl class="trnsl'
                        + str(chapter)
                    ...
                    outputline = re.sub("-##?", "", outputline)
                    outputline = re.sub("##?-", "", outputline)
                    if "<TR>" in line and ((prev_vsnum != vsnum and uvacaflag != 1) or proseflag == True):
                        collected_tr = (
                            collected_tr
                            + "\n<br/><br/>\n" + '<proseblock>'*proseflag
                    ...
                    if "</TR>" in line:
                        prev_vsnum = vsnum
                        trflag = False
                        collected_tr = collected_tr + outputline + '</proseblock>\n\n'*proseflag
